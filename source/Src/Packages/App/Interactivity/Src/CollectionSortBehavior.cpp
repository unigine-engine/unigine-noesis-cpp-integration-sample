...
////////////////////////////////////////////////////////////////////////////////////////////////////
static int BinarySearch(SortComparer* comparer, SortedCollection* list, Noesis::BaseComponent* item,
    int low, int high)
{
    if (comparer == nullptr)
        return high + 1;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void CollectionSortBehavior::ResetItems()
{
    ...
    if (NS_UNLIKELY(list == nullptr)) return;
...
void CollectionSortBehavior::SortItems()
{
    SortedCollection* sorted = GetSortedItems();
    SortComparer* comparer = GetComparer();
    if (comparer == nullptr) return;

    // using insertion sort algorithm
    int numItems = sorted->Count();
    for (int i = 1; i < numItems; ++i)
    {
        Noesis::BaseComponent* item = sorted->Get(i);

        // Move elements of the sorted collection in the range [0..i-1], that are greater than
        // current item, to one position ahead of their current position
        int j = i - 1;
        while (j >= 0 && comparer->Compare(sorted->Get(j), item) > 0)
        {
            j = j - 1;
        }
...
NS_IMPLEMENT_REFLECTION(CollectionSortBehavior, "NoesisGUIExtensions.CollectionSortBehavior")
{
    ...
    {
        CollectionSortBehavior* behavior = Noesis::DynamicCast<CollectionSortBehavior*>(d);
        if (behavior != nullptr)
        {
